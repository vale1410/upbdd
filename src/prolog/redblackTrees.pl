...
:- module(rbtrees,
          [ rb_new/1,
            rb_empty/1,                 % ?T
            rb_my_lookup/4,             % +Key, -Value, +T, [LB,UB]
...
rb_my_lookup(Key, Val, t(_,Tree),[A,B]) :-
        my_lookup(Key, Val, Tree,[A,B]).

%% my lookup is for the transformation algorithm,
% the key is an interval and the lookup checks if the
% current key is included in the interval, else
% continues searching


my_lookup(_, _, black([],_,_,[]),_) :- !, fail.
my_lookup(Key, Val, Tree,[LB,UB]) :-
      arg(2,Tree,[LB1,UB1]),
      (Key < LB1 ->
         (
         arg(1,Tree,Left),
         my_lookup(Key,Val,Left,[LB,UB])
         )
      ;
       Key > UB1 ->
         (
         arg(4,Tree,Right),
         my_lookup(Key,Val,Right,[LB,UB])
         )
      ;
         arg(3,Tree,Val),
         LB = LB1,
         UB = UB1
%         write(['node found(Value,LB,UB,DATA) ',Key,LB,UB|Val])
      ).
...
% insertion for the lookup for the transformation
% cmp is either
