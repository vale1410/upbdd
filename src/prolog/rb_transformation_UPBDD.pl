:-module(rb_transformation_UPBDD,[buildUPBDD/3]).
:-use_module(upbdd_JEAN,[assertUPBDD/4]).
%:-use_module(upbdd,[assertUPBDD/4]).
:-use_module(redblackTrees,[rb_new/1,rb_my_lookup/4,rb_insert/4]).
:-use_module(utils).

buildUPBDD(NormalisedCoefficients,Threshold,TOPNode):-
   T is -Threshold,
   buildBDD(NormalisedCoefficients,T,TOPNode,_LB,_UB,[],_ListOfTreesOUT).

buildBDD(_Coefficients,Value,1-1,0,1000000000000000,ListOfTreesIN,ListOfTreesIN):-
   Value >= 0,
   !.
buildBDD([],_Value,0-1,-1000000000000000,-1,ListOfTreesIN,ListOfTreesIN):- !.
...
insert(Level,LB,UB,0-1,0-1,0-1,ListOfTreesIN,ListOfTreesOUT):-
   insertion([Level,LB,UB],[0-1,0-1,0-1],ListOfTreesIN,ListOfTreesOUT),
